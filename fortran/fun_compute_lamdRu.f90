subroutine fun_compute_lamdRu(N_node,N_face,rho,N_GAUSS,Matrix_P0,F1,N_edge,C1,Fac_Ed_loc,Aed,Led,&
                            N_thread,lambda_vec,u_vec,lamDRu)
implicit none
...
real*8 lamDRu(N_face)
real*8 lambda_vec(N_edge), u_vec(N_edge)
!!
real*8 pi
...
real*8 we_glo(3,N_GAUSS**2,4), tmp3(3)
real*8 lam_loc, u_loc, temp1, temp2
real*8 Rcoeff
!!
pi=4.d0*datan(1.d0)
...
do ii = 1,N_face
    ...
    do kk = 1,4 
        ...
        ! extract corresponging lambda elements
	    lam_loc=lambda_vec(ind_e1)
        temp1=0.0d0
        do hh = 1,4
            ind_e2s=(C1(hh,ii))
            ind_e2=abs(ind_e2s)
            ind_loc_wh=Fac_Ed_loc(hh,ii)
            ! extract corresponding u elements
			u_loc=u_vec(ind_e2)
            Rcoeff=0.0d0
            do ee = 1,NP
            ...
            enddo
            temp1=temp1+Rcoeff*u_loc
        enddo
        temp2=temp2+lam_loc*temp1
    enddo
    lamDRu(ii)=lamDRu(ii)+temp2    
enddo
end subroutine fun_compute_lamdRu
! ---------------------------------------------------- !
