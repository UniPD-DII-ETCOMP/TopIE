...
! mwPointer stuff for mexing
	  ...
	  mwPointer N_edge_pr,C1_pr,Fac_Ed_loc_pr,Aed_pr,Led_pr,N_thread_pr,lamDRu_pr
      mwPointer lambda_vec_pr, u_vec_pr
      mwPointer m, n
...
! fortran subroutine arguments
	real*8,allocatable,dimension(:,:) :: Matrix_P0, F1_r, C1_r, Fac_Ed_loc_r
      integer*8,allocatable,dimension(:,:) :: F1,C1, Fac_Ed_loc
	real*8,allocatable,dimension(:) ::  Aed, Led, rho, lamDRu
      real*8,allocatable,dimension(:) :: lambda_vec, u_vec
	...
	character*80 msg
	  ...
	  if(debu) open(unit=66,file='log_lamdRu_axi.txt',status='unknown')
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	  
if (nrhs .ne. 14) then
        call mexErrMsgIdAndTxt ('MATLAB:fun_auto:nInput', &
                                '14 input arguments required')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!     Check that input #13 is INTEGER vector and fetch it
      m = mxGetM(prhs(13))
      n = mxGetN(prhs(13))
      if(m .ne. 1 .or. n .ne. N_edge) then
         call mexErrMsgIdAndTxt ('MATLAB:convec:NonRowVector', &
      ...
      siz = m*n
      lambda_vec_pr = mxGetPr(prhs(13))
      allocate(lambda_vec(N_edge))
      call mxCopyPtrToReal8(lambda_vec_pr, lambda_vec, siz)  
	if(debu) write(66,*) 'input 13'
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!     Check that input #14 is INTEGER vector and fetch it
      m = mxGetM(prhs(14))
      n = mxGetN(prhs(14))
      if(m .ne. 1 .or. n .ne. N_edge) then
         call mexErrMsgIdAndTxt ('MATLAB:convec:NonRowVector', &
      ...
      siz = m*n
      u_vec_pr = mxGetPr(prhs(14))
      allocate(u_vec(N_edge))
      call mxCopyPtrToReal8(u_vec_pr, u_vec, siz)  
	if(debu) write(66,*) 'input 14'    
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      	  	  
! call the computational subroutine
      allocate(lamDRu(N_face))
      call fun_compute_lamdRu(N_node,N_face,rho,N_GAUSS,Matrix_P0,F1,&
            N_edge,C1,Fac_Ed_loc,Aed,Led,N_thread,lambda_vec,u_vec,lamDRu)
...
! Load the output 1 into a MATLAB array.
      lamDRu_pr = mxGetPr(plhs(1))
      siz=mo*no
      call mxCopyReal8ToPtr(lamDRu, lamDRu_pr, siz)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
      if(debu) write(66,*)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
      deallocate(lamDRu)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!     
      if(debu) write(66,*) 'close log_lamdRu_axi.txt'
      if(debu) close(66)
      return
      end
