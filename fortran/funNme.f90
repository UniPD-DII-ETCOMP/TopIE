subroutine  funNme(N_node,N_face,Area,N_GAUSS1,N_point_m,Matrix_P0,F1,N_thread,Matrix_P0_m,L)
implicit none
integer*8 N_node, N_face, N_GAUSS1, F1(4,N_face)
real*8 Area(N_face), Matrix_P0(N_node,3), L(N_point_m,N_face), Matrix_P0_m(N_point_m,3)
...
integer*8 hh, ii, jj, kk, qq, ee, N_point_m
real*8 Hexa1(8,3), Hexa2(8,3)
real*8 PP1(N_GAUSS1**2,3)
real*8 detJ1(N_GAUSS1**2)
real*8 J1(3,3), J2(3,3)
real*8 Face1(4,3), Face2(4,3), dett, cent(3)
real*8 dr, dz, rm, Lself, Lint, ree, rqq, h
real*8 Kg2, k2, Kg, k, e1, e2, G2Daxi, pi, xyz(3), Pm(3)
!!
!do ii=1,N_face
...
enddo
!!
L(1:N_point_m,1:N_face)=0.0d0
call omp_set_num_threads(N_thread)
!$OMP PARALLEL SHARED(Matrix_P0_m,N_point_m,N_face,Matrix_P0,F1,NP1,xi1,eta1,zet1,pi,Area,WG_loc1,L)
!$OMP DO SCHEDULE(DYNAMIC,1) PRIVATE(Pm,ii,Face1,Hexa1,kk,xyz,PP1,J1,dett,jj, &
!$OMP                                detJ1,Lint,qq,ee,ree,rqq,h,Kg2,k2,Kg,k,e1,e2,G2Daxi)
...
!! 
    do jj = 1,N_point_m
        ...
        do qq = 1,NP1
                ...
                Lint=Lint+  rqq*G2Daxi*WG_loc1(qq)*detJ1(qq)
        enddo
        Lint=2.0d0*pi*Lint/(Area(ii))
        L(jj,ii)=Lint
    enddo
enddo
!$OMP END DO NOWAIT 
!$OMP END PARALLEL 
end subroutine funNme
!!
