subroutine  funPphiphi3(N_node,N_face,Area,N_GAUSS1,N_GAUSS2,Matrix_P0,F1,N_thread,Area_vol,P)
implicit none
integer*8 N_node, N_face, N_GAUSS1, N_GAUSS2, F1(4,N_face)
real*8 Area(N_face), Matrix_P0(N_node,3), P(N_face,N_face)
real*8 Area_vol(N_face)
...
real*8 dr, dz, rm, Pint, ree, rqq, h
real*8 Kg2, k2, Kg, k, e1, e2, G2Daxi, pi, xyz(3)
!!
pi=4.d0*datan(1.d0)
!!
call gauleg(N_GAUSS1,xabsc1(1:N_GAUSS1),weig1(1:N_GAUSS1))
NP1=N_GAUSS1**2  
hh=1
eta1(1:NP1)=0.0d0
do ii = 1,N_GAUSS1 ! 
    do jj = 1,N_GAUSS1
           xi1(hh) =xabsc1(ii)
           zet1(hh)=xabsc1(jj)
           WG_loc1(hh)= weig1(ii)*weig1(jj) 
           hh=hh+1
    enddo 
enddo
!!
call gauleg(N_GAUSS2,xabsc2(1:N_GAUSS2),weig2(1:N_GAUSS2))
NP2=N_GAUSS2**2
hh=1
eta2(1:NP2)=0.0d0
do ii = 1,N_GAUSS2 ! 
...
enddo
!!
P(1:N_face,1:N_face)=0.0d0
call omp_set_num_threads(N_thread)
!$OMP PARALLEL SHARED(N_face,Matrix_P0,F1,NP1,xi1,eta1,zet1,NP2,xi2,eta2,zet2,pi,Area,WG_loc1,WG_loc2,Area_vol,P)
!$OMP DO SCHEDULE(DYNAMIC,1) PRIVATE(ii,Face1,Hexa1,kk,xyz,PP1,J1,dett,jj,Face2,Hexa2,PP2, &
!$OMP                                J2,detJ2,detJ1,Pint,qq,ee,ree,rqq,h,Kg2,k2,Kg,k,e1,e2,G2Daxi)
do ii = 1,N_face ! ciclo sulle facce
    Face1(1,1:3)=Matrix_P0(F1(1,ii),1:3)
    Face1(2,1:3)=Matrix_P0(F1(2,ii),1:3)
    Face1(3,1:3)=Matrix_P0(F1(3,ii),1:3)
    Face1(4,1:3)=Matrix_P0(F1(4,ii),1:3)
    ...
    do jj = ii,N_face
        ...
        do qq = 1,NP1
            do ee = 1,NP2
                ...
                Pint=Pint+ G2Daxi*WG_loc1(qq)*WG_loc2(ee)*detJ1(qq)*detJ2(ee)			
				!!
            enddo
        enddo
        Pint=Pint/(Area(ii)*Area_vol(jj))
        P(ii,jj)=Pint
        P(jj,ii)=Pint
    enddo
enddo
