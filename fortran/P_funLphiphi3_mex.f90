...
! mwPointer stuff for mexing
      ...
      mwPointer mxGetPr
	  mwPointer N_node_pr,N_face_pr,Area_pr,N_GAUSS1_pr,N_GAUSS2_pr,Matrix_P0_pr,F1_pr,N_thread_pr,Area_vol_pr,L_pr
      mwPointer m, n
...
!	  integer*8,allocatable,dimension(:) :: 
	  real*8 :: N_node_r, N_face_r, N_GAUSS1_r, N_GAUSS2_r, N_thread_r
      integer*8 :: N_node, N_face, N_GAUSS1, N_GAUSS2, N_thread
	  character*80 msg
      logical debu
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!     Check that input #9 is INTEGER vector and fetch it
      m = mxGetM(prhs(9))
      n = mxGetN(prhs(9))
      if(m .ne. 1 .or. n .ne. N_face) then
         call mexErrMsgIdAndTxt ('MATLAB:convec:NonRowVector', &
                                'Input 9: Area_vol: 1xN_face')
      endif	  
      siz = m*n
      Area_vol_pr = mxGetPr(prhs(9))
...
! call the computational subroutine.
      allocate(L(N_face,N_face))
      call funPphiphi3(N_node,N_face,Area,N_GAUSS1,N_GAUSS2,Matrix_P0,F1,N_thread,Area_vol,L)
       if(debu) write(66,*) 'ho chiamato la subroutine e creato le matrici'
      deallocate(Area,Matrix_P0,F1)
